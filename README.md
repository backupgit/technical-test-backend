...
To populate db data uncomment this block in Dockerfile and run it once
...
- Populate MongoDB: 
```
mongoimport --host localhost --db city-population --collection cities --type json --file mongo-seed/mongo-seed.json --jsonArray
```
- `npm i`
- `npm start`
...
- Insert population data (POST) `/cities`

- Population by city (its last record) and age `/cities`

- Population by city `/cities/population/{city}`

- Population by all ages `/cities/population`

- Population by cities (of all time) `/cities/population/ages`
